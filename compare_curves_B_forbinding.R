...
a = a[order(a$Date), ]
a = a[, c('context', 'hole', 'env', 'user', 'prog', 'session', 'cage', 'pass', 'ratID', 'Date', 'hour', bin.names)]
...
means = ddply(a, .(hole), .fun=m.mean, 13:ncol(a))
sems = ddply(a, .(hole), .fun=std, 13:ncol(a))

means = melt(means, id.vars=c('hole'), value.name='bin')
colnames(means) = c('hole', 'bin', 'Mean')
sems = melt(sems, id.vars=c('hole'), value.name='bin')
colnames(sems) = c('hole', 'bin', 'SEM')

dd = merge(means, sems, by=1:3)
a.plot = ggplot(data=dd, aes(x=bin, y=Mean, linetype=factor(hole))) +
         geom_ribbon(
                     aes(ymin=Mean-SEM, ymax=Mean+SEM, group=interaction(hole)),
                     alpha=0.2, size=0.2
                    ) +
